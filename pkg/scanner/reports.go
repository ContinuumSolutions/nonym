...
func generateReport(report *Report, baseURL string) {
	...
	pdfBytes, err := renderPDF(report, findings)
	if err != nil {
...
func renderPDF(report *Report, findings []Finding) ([]byte, error) {
	...
	pdf.SetFooterFunc(func() {
		...
		pdf.CellFormat(0, 10,
			fmt.Sprintf("Nonym Privacy Gateway — %s Compliance Report — Page %d", report.Framework, pdf.PageNo()),
			"", 0, "C", false, 0, "")
	})
	...
	pdf.RoundedRect(20, 60, 170, 60, 3, "1234", "F")

	pdf.SetFont("Helvetica", "B", 11)
	pdf.SetXY(30, 68)
	pdf.Cell(50, 8, "Framework:")
	pdf.SetFont("Helvetica", "", 11)
	pdf.Cell(100, 8, report.Framework)

	pdf.SetFont("Helvetica", "B", 11)
	pdf.SetXY(30, 78)
	pdf.Cell(50, 8, "Time Range:")
	pdf.SetFont("Helvetica", "", 11)
	pdf.Cell(100, 8, report.TimeRange)

	pdf.SetFont("Helvetica", "B", 11)
	pdf.SetXY(30, 88)
	pdf.Cell(50, 8, "Generated:")
	pdf.SetFont("Helvetica", "", 11)
	pdf.Cell(100, 8, time.Now().Format("02 Jan 2006, 15:04 UTC"))

	pdf.SetFont("Helvetica", "B", 11)
	pdf.SetXY(30, 98)
	pdf.Cell(50, 8, "Report ID:")
	pdf.SetFont("Helvetica", "", 11)
	pdf.Cell(100, 8, report.ID)
	...
	pdf.SetTextColor(31, 41, 55)
	pdf.SetX(20)
	pdf.MultiCell(170, 6, frameworkSummary(report.Framework), "", "L", false)
...
func renderFinding(pdf *fpdf.Fpdf, idx int, f Finding) {
	...
	pdf.SetTextColor(31, 41, 55)
	pdf.SetX(20)
	pdf.CellFormat(130, 7, fmt.Sprintf("%d. %s", idx, f.Title), "LTB", 0, "L", true, 0, "")
	...
	pdf.SetX(20)
	pdf.CellFormat(170, 6, fmt.Sprintf("Vendor: %s  |  Data Type: %s  |  Status: %s  |  Occurrences: %d",
		f.Vendor, f.DataType, f.Status, f.Occurrences), "LRB", 1, "L", true, 0, "")

	if f.Description != "" {
		pdf.SetX(20)
		pdf.MultiCell(170, 5, f.Description, "LRB", "L", false)
	}
	...
	if len(f.ComplianceImpact) > 0 {
		...
		pdf.SetFont("Helvetica", "I", 9)
		pdf.SetTextColor(107, 114, 128)
		pdf.MultiCell(170, 5, "Compliance: "+strings.Join(articles, ", "), "LRB", "L", false)
	}
